...
- [x] Editar incidencia (`PUT /api/issues/{id}` + formulario) ← primera tarea sugerida, rama `feature/editar-incidencia`
- [x] Eliminar incidencia (`DELETE /api/issues/{id}` + botón en el listado)
- [x] Inicio de sesión con JWT (`POST /api/auth/login`, Bearer tokens y guard/interceptor Angular)
- [x] Gestión básica de usuarios (registro, consulta y administración restringida por rol)
- [x] Creación y consulta de proyectos (CRUD, miembros y frontend en `feature/gestion-proyectos`)
